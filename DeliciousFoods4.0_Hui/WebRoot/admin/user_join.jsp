	...
	<div id="container_demo">
		...
		<div id="wrapper">
			...
			<div class="animate form" id="register">
				<form action="users_save" id="usersFrm" autocomplete="on" method="post">
					<input type="hidden" name="id" value="${id}"/>
					<input type="hidden" name="users.id" value="${users.id}"/>
					<h1>注册</h1>
					<p>
						<label class="uname" data-icon="u" for="usernamesignup">用户名</label>
						<input id="username" name="${users.userName }" placeholder="用户名"
							required="required" type="text" />
					</p>
					<p>
						<label class="youpasswd" data-icon="p" for="passwordsignup">密码</label>
						<input id="passwordsignup" name="${users.password}" placeholder="密码"
							required="required" type="password" />
					</p>
					<p>
						<label class="youpasswd" data-icon="p" for="passwordsignup2">再次输入</label>
						<input id="passwordsignup2" name="passwordB" placeholder="密码"
							required="required" type="password" onmouseout="checkpwd()" />
					</p>
					<p>
						<input type="checkbox" onclick="change()" id="changepsw" />显示密码
					</p>
					...
					<p class="change_link">
						已经是成员?<a class="to_register" href="index"> 去登录 </a> <a
							class="to_register" href="#userFrm">去测试</a>
					</p>
				</form>
			</div>
		</div>
